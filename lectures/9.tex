...
\begin{outline}
    \1 Vanilla — The basic form of gradient descent where the entire dataset is used to compute the gradient in each iteration. \textit{Use: simple and stable for small datasets.}
    \1 Stochastic — Uses a single random data point to update the weights in each iteration. \textit{Use: faster updates and more noise, good for large datasets.}
    \1 Minibatch — Uses a small batch of data points to compute the gradient. \textit{Use: balances stability and speed, commonly used in practice.}
    \1 Hessian — Refers to second-order optimization using the Hessian matrix of second derivatives. \textit{Use: captures curvature for more accurate updates but is computationally expensive.}
    \1 Momentum — Accelerates gradient descent by adding a fraction of the previous update to the current one. \textit{Use: helps escape local minima and smoothens convergence.}
